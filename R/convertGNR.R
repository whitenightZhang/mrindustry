...
convertGNR <- function(x) {
  ...
  no_remove_warning <- c("Morocco + Algeria + Tunisia", "EU27", "South/Latin America", "World", # are disaggregated manually
  ...
  left_countries <- getRegions(x_out)[!getRegions(x_out) %in% set_countries]
