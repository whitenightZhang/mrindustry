...
#'
calcChemicalFeedstocks <- function() {
  # Read in Final Energy (FE) data from IEA and convert from ktoe to EJ
  iea <- readSource("IEA", subtype = "EnergyBalances", convert = TRUE)[, , "NECHEM"] %>%
  ...
  for (g in unique(product_mapping$group)) {
    ...
    CHEM.FEEDSTOCK[, , g] <- dimSums(iea[, , products], dim = 3, na.rm = TRUE)
  }
